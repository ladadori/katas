...
export default (totalMinuteCount) => {
  ...
  totalMinuteCount > minutesInDay ? (hoursPassed -= hoursInDay) : hoursPassed;

  const hourCountFormatted =
    hoursPassed < 10 ? `${0}${hoursPassed}` : `${hoursPassed}`;

  const minuteCountFormatted =
